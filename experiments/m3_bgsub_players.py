"""M3 experiment 4 — $0 player tracking: stabilize, median, subtract.
...
Usage:
    ...
    (no flag: both passes; no clips: all of clips/points/)
"""
...
def main():
    ...
    paths = ([Path(p) for p in args.clips] if args.clips
             else sorted((ROOT / "clips/points").glob("point_*.mp4")))
    all_paths = sorted((ROOT / "clips/points").glob("point_*.mp4"))
