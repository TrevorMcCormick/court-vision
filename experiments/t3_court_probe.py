"""T3 — court-view detection on clay: the t2 probe redesigned.

The t1/t2 discriminator was interior-probes-BLUE and apron-probes-NOT-
blue. Clay kills it twice: the court isn't blue, and the RG apron is
the SAME clay as the court — "apron reads a different color" is a
Montreal fact, not a tennis fact. So the apron test is replaced with a
LINE test, which is geometric rather than colorimetric:

  interior probes — the t2 grid projected into the image must read
                    clay (dilated mask; players/lines cover a few)
  line probes     — points sampled along the projected model lines
                    (far baseline excluded: dust-buried) must read
                    thin-bright (tophat mask, same as the fit). A
                    close-up shot puts clay under the interior probes
                    but cannot put white lines in the right geometry.

court view  <=>  interior >= INT_MIN and lines >= LINE_MIN.

One more clay lesson baked in: this camera PANS (up to ~25 px between
points), and a fixed projection walks the line probes off the real
lines — known court view read line_hit 0.29 and the whole reel came
back 68 s of play. The line read is therefore a MAX over a small grid
of probe shifts (+-24 px x, +-16 px y full-res); a shifted court view
recovers 0.44-0.72 while close-ups stay ~0.1. The winning shift is
recorded per frame — downstream stages need it to map back to the fit
camera's coordinates.

Then segment: median-smooth the boolean, keep runs >= MIN_RUN_S,
merging gaps <= GAP_S.

Usage:
    uv run experiments/t3_court_probe.py clips/t3_djokovic_ruud_30fps.mp4
"""
...
OUT_DIR = Path(__file__).resolve().parent.parent / "outputs" / "t3"
...
def main():
    ...
    Hc2i = np.load(ROOT / "outputs/t3/H_court_to_img.npy")
