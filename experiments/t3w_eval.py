"""T3 — the clay scorecard: t2w_eval with t3 paths and RG-final structure.

Match 20230611 RG F: player 1 = Ruud, player 2 = Djokovic; 7-6(1) 6-3
7-5. Set priors for changeover parity: set 1 ran 12 games + tiebreak
(counts 13), set 2 nine games, so set-2 clips start 13 games deep and
set-3 clips 22. Both players right-handed — letters compare straight.
START_END is the parity-vote winner from staging (see LOG).

Usage:
    uv run experiments/t3w_eval.py
"""
...
DATA = ROOT / "data" / "mcp"
CHART = ROOT / "outputs" / "t3" / "charts_wasb"
...
START_END = "far"           # parity vote 9/14 over near 5/14, corroborated
                              # by frame: Djokovic (red) is the NEAR player in
                              # game 1 while Ruud serves


def p1_end(a):
    """Which end (near/far) player 1 (Ruud) occupies for this clip's game.

    Set priors from the MCP rows themselves: set 1 = 12 games + tiebreak
    (13 swap-units), set 2 = 9 games. Tiebreak clips add end changes
    every 6 points, t2 recipe."""
    g = int(a["gm1"]) + int(a["gm2"])
    prior = 0
    if a["set2"] == "1":
        prior = 13                # after set 1 (12 games + TB)
    elif a["set2"] == "2":
        prior = 22                # after sets 1 and 2 (13 + 9)
    g += prior
    swaps = (g + 1) // 2
    if a["gm1"] == "6" and a["gm2"] == "6" and a["set1"] == "0" and a["set2"] == "0":
        p1, p2 = a["pts"].split("-")
        swaps += (int(p1) + int(p2)) // 6
    other = {"near": "far", "far": "near"}
    return other[START_END] if swaps % 2 else START_END


def main():
    mapd = {r["clip"]: r for r in csv.DictReader(open(DATA / "t3_mcp_map.csv"))}
    align = {r["clip"]: r for r in csv.DictReader(open(DATA / "t3_clip_alignment.csv"))}
    ...
    print("\n=== scorecard (t3: clay, RG world feed) ===")
    s = tally
