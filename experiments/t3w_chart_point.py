"""T3 — chart loop on WASB tracks, clay adversarial match (RG 2023 final,
Djokovic-Ruud, both right-handed). Byte-twin of t2w_chart_point.py except
paths, plus ONE addition for the wandering RG camera: ball pixels are
shifted by the clip-level offset (outputs/t3/clip_offsets.csv, measured
by the probe's shift search) before the court projection, so landings
and the envelope gate are computed in the fit camera's frame. All
frozen-era constants and the freeze-#3 envelope gate are untouched.
...
Usage:
    uv run experiments/t3w_chart_point.py t3_point_01 ...
"""
...
# PLAYER SOURCE: "players" = the $0 bgsub tracks (default);
# "players_sam" = SAM-3 via fal (t3_sam_players.py, ~$0.15/clip) — the
# buy-vs-build A/B for the letter sink. Same schema, same hygiene.
PLAYERS_DIR = "players"
...
# (t4's coda-truncation pass does NOT come to t3: measured net negative
# here — clay's failure mode is undercounting and the anchor pays
# crossings-recall it doesn't have. Receipt in the LOG.)
...
def assign_strikers(shots, server, holes, lock=None):
    ...
    CORRECT end to wrong; t3's ball-launch end read 48/56 against
    parity truth while the boxes chase clay shadows)."""
...
def chart_clip(stem, Hm, serves):
    clip = ROOT / "clips/points_t3" / f"{stem}.mp4"
    ...
    # box hygiene (player_boxes.py): depth-consistent height gate,
    # court-half plausibility, teleport rejection, short-gap interp —
    # the audit-measured letter sink lives in the raw boxes
    players = player_boxes.load(
        OUT_BASE / PLAYERS_DIR / f"players_{stem}.csv", Hm,
