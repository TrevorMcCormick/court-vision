"""T1 — extract candidate point clips: m3_extract_points with t1 paths.
...
Usage:
    uv run experiments/m3_extract_points.py clips/match_r2_highlights.mp4
"""
...
OUT_DIR = Path(__file__).resolve().parent.parent / "outputs" / "t1"
ROOT = Path(__file__).resolve().parent.parent
POINTS_DIR = ROOT / "clips" / "points_t1"
...
def main():
    ...
    if not args.skip_clips:
        ...
        for s in segs:
            ...
            dur = float(s["dur_s"])
            out = POINTS_DIR / f"t1_point_{k:02d}.mp4"
            subprocess.run(
