"""T3 (Djokovic-Ruud, Roland Garros 2023 final) — fit the court homography.

T3 is the CLAY adversarial test, and the M1/t2 auto recipe died three
ways before this version worked (all receipts in LOG.md):

  1. "Blue court hull" has no blue: the clay reads H 6-8 — and so does
     sun-lit CROWD SKIN, so a plain hue hull swallowed the whole frame.
     Fix: tighter clay band (S>=60, V>=120 kills shaded crowd), largest
     connected component, FILLED CONTOUR instead of convex hull (the
     convex hull bridged into the bottom crowd row).
  2. The unstabilized median plate SMEARS the lines into the clay (this
     camera pans ~16 px over the fit run; Montreal's white-on-blue
     survived the smear, dusted white-on-orange does not). Fix: ECC-
     stabilize the plate frames first (t2_bgsub recipe).
  3. The far baseline is buried in clay dust (V ~200 on a ~195 clay —
     no HSV mask finds it) and the apron junk (Perrier crates, Haier
     box: 20 Hough segments!) lives INSIDE the clay hull, so "extreme
     clusters = baselines + doubles lines" picks garbage. Fixes:
     lines via TOPHAT (thin-bright) not absolute white; verticals
     filtered by symmetry about the center-service cluster (junk is
     asymmetric); horizontal labels chosen by enumerating every
     order-preserving assignment of model lines onto clusters, LS-fit
     on all labeled intersections, scored by how well ALL NINE model
     lines land on the observed line mask (fit rms cannot choose — a
     wrong-but-self-consistent triple fit its own points to rms 1.7 px
     on t4; mask coverage strands the lines an impostor didn't use).

Validation: residuals per model line against the mask are printed, the
dust-buried far baseline is reported for the record, and the
reprojection overlay gets LOOKED at.

All t3 artifacts go to outputs/t3/.

Usage:
    uv run experiments/t3_fit_homography.py clips/t3_djokovic_ruud_30fps.mp4 --lo 36240 --hi 36540
    uv run experiments/t3_fit_homography.py ... --manual   # corner-constant fallback
"""
...
SERVICE_FAR_Y = 5.485          # court-model y of the far service line

CLAY_LO, CLAY_HI = (0, 60, 120), (15, 255, 255)
TOPHAT_K = 25                  # thin-bright structure width, px
TOPHAT_T = 25
V_MIN = 170
HULL_ERODE = 15
SYM_TOL = 12                   # px; vertical-pair midpoint vs center line
MIN_SUPPORT = 3                # segments per cluster to be trusted
...
def corner_crops(plate, corners, zoom=6):
    """Gridded zoom crops of the four fit-point regions for manual reads."""
...
def main():
    ...
    # ---- clay hull: largest component, filled contour (NOT convex) ----
    clay = cv2.inRange(hsv, CLAY_LO, CLAY_HI)
    clay = cv2.morphologyEx(clay, cv2.MORPH_CLOSE, np.ones((25, 25), np.uint8))
    n, labels, stats, _ = cv2.connectedComponentsWithStats(clay)
    ...
    # ---- line mask: thin-bright tophat inside the eroded hull ----
    v = hsv[:, :, 2]
    ...
    # ---- validation: measured clusters vs the projection ----
    checks = {
    ...
    for name, (pt, axis, obs) in checks.items():
        ...
        best_v = min(obs, key=lambda q: abs(q - proj[axis])) if obs else None
        d = abs(best_v - proj[axis]) if best_v is not None else float("nan")
        print(f"validate {name}: projected {proj[axis]:.1f}, "
              f"nearest measured {best_v}, err {d:.1f} px")

    # ---- held-out lines vs nearest line-mask pixel ----
    dist_all = cv2.distanceTransform(255 - line_mask, cv2.DIST_L2, 5)
    held_out = ["singles_left", "singles_right", "baseline_far",
    ...
    for name in held_out:
        ...
        print(f"held-out {name}: mean dist to line px = {d.mean():.1f} px "
              f"(max {d.max():.1f})")
