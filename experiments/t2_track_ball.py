"""T2 — ball tracking on the test match: m3_track_ball, t1 paths.
...
Usage:
    uv run experiments/t2_track_ball.py point_16 point_25 ... --dry-run
    uv run experiments/t2_track_ball.py point_16 point_25 ... --send
"""
...
BALL_AREA = (6, 220)
W_C, L_C = 10.97, 23.77
SERVER_Y_BAND = {"far": (-4.0, 8.0), "near": (16.0, 28.0)}   # court m, foot
...
def toss_bootstrap(stem, serve, players, shifts, plate, clip_path):
    ...
    t1 lesson: this match's ballkids crouch at the NET POSTS, inside the
    far half, and the largest-component player pick grabs them frame to
    frame — the toss window then opens above a ballkid's head (or the
    umpire chair). A server stands at his baseline: gate each frame's
    player box by foot position in court meters before trusting it."""
    Hm = np.load(ROOT / "outputs/t2/H_img_to_court.npy")
    y_lo, y_hi = SERVER_Y_BAND[serve["server"]]
    ...
    for fi in range(sf - 2, max(0, sf - 18) - 1, -1):
        if fi not in frames or fi not in players or side not in players[fi]:
            continue
        p = players[fi][side]
        foot = np.float32([[float(p["foot_x"]), float(p["foot_y"])]]).reshape(-1, 1, 2)
        fy = float(cv2.perspectiveTransform(foot, Hm).reshape(2)[1])
        if not (y_lo <= fy <= y_hi):
            continue          # that's not a server, that's a ballkid
        cx = float(p["cx"]) * 1280
        h_px = float(p["h"]) * 720
...
def mover_bootstrap(stem, players, shifts, plate, clip_path, n_frames, region):
    ...
    overlay[595:680, 95:360] = 0     # score bug (Sky feed, same corner)
    overlay[655:705, 470:810] = 0    # MONTREAL 375 watermark
    overlay[265:335, 230:1050] = 0   # net band + Fly Emirates post panels
    overlay[15:80, 1040:1250] = 0    # Sky Sports logo top-right
...
def court_region(shape):
    Hc2i = np.load(ROOT / "outputs/t2/H_court_to_img.npy")
...
def main():
    ...
    for stem in args.clips:
        clip_path = ROOT / "clips/points_t2" / f"{stem}.mp4"
