"""T1 — $0 player tracking on the test match: m3_bgsub_players, t1 paths.
...
Usage:
    uv run experiments/m3_bgsub_players.py --pass-a [clips...]
    uv run experiments/m3_bgsub_players.py --pass-b [clips...]
    (no flag: both passes; no clips: all of clips/points_t1/)
"""
...
def main():
    ...
    paths = ([Path(p) for p in args.clips] if args.clips
             else sorted((ROOT / "clips/points_t1").glob("t1_point_*.mp4")))
    all_paths = sorted((ROOT / "clips/points_t1").glob("t1_point_*.mp4"))
