"""T1 — serve detection on the test match: m3_serve_detect, t1 paths.
...
Usage:
    uv run experiments/m3_serve_detect.py
"""
...
def main():
    ...
    rows_out = []
    tracks = sorted(TRACK_DIR.glob("players_t1_point_*.csv"))
    for tpath in tracks:
        stem = tpath.stem.replace("players_", "")
        cap = cv2.VideoCapture(str(ROOT / "clips/points_t1" / f"{stem}.mp4"))
        ...
        def court_xy(r):
            pt = np.float32([[float(r["foot_x"]), float(r["foot_y"])]]).reshape(-1, 1, 2)
