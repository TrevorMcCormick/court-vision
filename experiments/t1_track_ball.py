"""T1 — ball tracking on the test match: m3_track_ball, t1 paths.
...
OUT_BASE = Path(__file__).resolve().parent.parent / "outputs" / "t1"
...
def toss_bootstrap(stem, serve, players, shifts, plate, clip_path):
    ...
    Hm = np.load(ROOT / "outputs/t1/H_img_to_court.npy")
...
def court_region(shape):
    Hc2i = np.load(ROOT / "outputs/t1/H_court_to_img.npy")
...
def main():
    ...
    for stem in args.clips:
        clip_path = ROOT / "clips/points_t1" / f"{stem}.mp4"
