"""T4 — the grass/WTA scorecard: t2w_eval with t4 paths and Wimbledon-
final structure.

Match 20240713 Wimbledon F: player 1 = Paolini, player 2 = Krejcikova;
6-2 2-6 6-4 to Krejcikova. Set priors: set-2 clips start 8 games deep,
set-3 clips 16. Both players right-handed. START_END is the parity-vote
winner from staging (see LOG).

Usage:
    uv run experiments/t4w_eval.py
"""
...
DATA = ROOT / "data" / "mcp"
CHART = ROOT / "outputs" / "t4" / "charts_wasb"
...
START_END = "far"           # parity vote 21/25 over near 4/25 — decisive


def p1_end(a):
    """Which end (near/far) player 1 (Paolini) occupies for this clip's game.

    Set priors from the MCP rows: set 1 = 8 games, set 2 = 8 games.
    No tiebreaks in this final."""
    g = int(a["gm1"]) + int(a["gm2"])
    prior = 0
    if a["set1"] == "0" and a["set2"] == "1":
        prior = 8                 # after set 1
    elif a["set1"] == "1" and a["set2"] == "1":
        prior = 16                # after sets 1 and 2
    g += prior
    swaps = (g + 1) // 2
...
def main():
    mapd = {r["clip"]: r for r in csv.DictReader(open(DATA / "t4_mcp_map.csv"))}
    align = {r["clip"]: r for r in csv.DictReader(open(DATA / "t4_clip_alignment.csv"))}
    ...
    print("\n=== scorecard (t4: grass, WTA, Wimbledon feed) ===")
    s = tally
