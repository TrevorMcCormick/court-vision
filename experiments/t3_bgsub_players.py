"""T3 — $0 player tracking: m3_bgsub_players, t3 paths.
...
Usage:
    uv run experiments/t3_bgsub_players.py --pass-a [clips...]
    uv run experiments/t3_bgsub_players.py --pass-b [clips...]
    (no flag: both passes; no clips: all of clips/points_t3/)
"""
...
def court_region_mask(shape):
    ...
    # t3 finding: the NET TAPE region is excluded from both halves. The
    # RG camera's residual jitter makes the high-contrast white band diff
    # against the plate every frame — under the 0.90 static-erase
    # threshold — and the tape blob out-areas the real far player ("far
    # player" at court y 10.8 = a box on the net, frame-checked). The
    # first cut at court y 11.0 missed most of it: the tape hangs ~1.07 m
    # ABOVE the ground plane, so its back-projection lands at court y
    # ~7-11.5, metres BEHIND the net line. Far half ends at 6.5; the
    # price is far players whose feet sit within ~5 m of the net
    # (approach shots lose their box at the volley).
    near = poly_mask([[-3.5, 12.8], [W_C + 3.5, 12.8],
...
def main():
    ...
    paths = ([Path(p) for p in args.clips] if args.clips
             else sorted((ROOT / "clips/points_t3").glob("t3_point_*.mp4")))
    all_paths = sorted((ROOT / "clips/points_t3").glob("t3_point_*.mp4"))
