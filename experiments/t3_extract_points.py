"""T3 — extract candidate point clips: t2_extract_points with t3 paths,
plus one clay-broadcaster addition: clip_offsets.csv.

The RG camera wanders point to point (the probe's shift search exists
...
Usage:
    uv run experiments/t3_extract_points.py clips/t3_djokovic_ruud_30fps.mp4
"""
...
POINTS_DIR = ROOT / "clips" / "points_t3"
...
def main():
    ...
    if not args.skip_clips:
        ...
        for s in segs:
            ...
            dur = float(s["dur_s"])
            out = POINTS_DIR / f"t3_point_{k:02d}.mp4"
            subprocess.run(
