"""T2 — serve detection on the test match: m3_serve_detect, t2 paths.
...
Usage:
    uv run experiments/t2_serve_detect.py
"""
...
def main():
    ...
    rows_out = []
    tracks = sorted(TRACK_DIR.glob("players_t2_point_*.csv"))
    for tpath in tracks:
        stem = tpath.stem.replace("players_", "")
        cap = cv2.VideoCapture(str(ROOT / "clips/points_t2" / f"{stem}.mp4"))
