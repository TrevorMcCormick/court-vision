"""T2 — court-view detection for the test match: m3_court_probe with t2 paths.
...
t2 finding: the probe grid's center column (court x=5.485) runs ALONG
the center service line and the x=1.5 columns sit 13 cm from the
singles lines — under day-session glare the white lines bloom wide and
those probes read white (V~250, S~25), not blue. Worse, the homography
was fit on a court-view run, so on exactly those frames the lines land
pixel-perfect on the probes: court_hit 0.68 on obvious court view.
Night exposure (t1) never blew the lines out, so the single-pixel read
survived there by luck. Fix: interior probes index a DILATED blue mask
(a probe on/next to a white line still counts as court); apron probes
keep the raw mask (dilation radius ~3 px at half res cannot reach 1.6 m
outside the lines, and a white close-up has no blue anywhere near).

Then segment: median-smooth the boolean, keep runs >= MIN_RUN_S, merging
gaps <= GAP_S.

Usage:
    uv run experiments/t2_court_probe.py clips/t2_federer_haase_30fps.mp4
"""
...
def main():
    ...
    while True:
        ...
        blue_dil = cv2.dilate(blue, np.ones((7, 7), np.uint8))
        court_hits.append(float(np.mean(blue_dil[p_int[:, 1], p_int[:, 0]] > 0)))
