"""M3 — extract candidate point clips + session artifacts.
...
POINTS_DIR = ROOT / "clips" / "points"

ZOOM_BLOCK = (93.0, 224.0)   # early set-1 tighter framing; probes reject it
MONTAGE_S = 2.0
...
def main():
    ...
    if not args.skip_clips:
        ...
        for s in segs:
            ...
            dur = float(s["dur_s"])
            out = POINTS_DIR / f"point_{k:02d}.mp4"
            subprocess.run(
