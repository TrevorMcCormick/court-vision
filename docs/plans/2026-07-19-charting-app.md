...
function press(ch){
  ...
  if(entry.state==="serve"){
    ...
    if(V.shot_letters.includes(ch) || "123".includes(ch)
      ...
      send("palette_key",{k:ch}); accept(); return; }
    else if(V.error_letters.includes(ch)){
      tokens().push(ch); entry.state="err-mark"; }
    else return;
  } else if(entry.state==="err-mark"){
    if(ch!=="@" && ch!=="#") return;
    tokens().push(ch); send("palette_key",{k:ch}); accept(); return;
  }
...
function exportBundle(){
  ...
  const scx=new JsScore(S.setup.best_of, S.setup.final_set,
    S.setup.first_server);
  const rows=S.points.map(p=>{
    const ctx=scx.rowContext(); scx.point(+p.PtWinner);
    return cols.map(c=>{
      ...
      if(c==="Notes") return (p.flags==="unseen"?"unseen;":"")+
        (p.notes||"");
      if(c==="PtWinner") return p.PtWinner;
      return ctx[c]??"";
  ...
  const segs=S.points.filter(p=>p.start_s!==""&&p.start_s!=null
    &&p.end_s!==""&&p.end_s!=null&&+p.start_s<+p.end_s).map(p=>
